...
def save_pickle_compress(object, filename, bin=1):
    """Saves a compressed object to disk
    """
    print("Original size: ", os.path.getsize(filename))
    file = gzip.GzipFile(filename, 'wb')
    file.write(pickle.dumps(object, bin))
    file.close()
    print("Compressed size: ", os.path.getsize(filename))


def load_pickle_compress(filename):
